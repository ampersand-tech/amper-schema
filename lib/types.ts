...
ACCOUNTIDSTR.truncate = truncateString;
...
PERSONAIDSTR.truncate = truncateString;
...
SHORTSTR.truncate = truncateString;
...
SHORTSTR_LOWERCASE.truncate = truncateString;
SHORTSTR_LOWERCASE._sqlTypeName = SHORTSTR._sqlTypeName;
...
JSONBLOB._sqlTypeName = LONGSTR._sqlTypeName;
JSONBLOB_NULLABLE._sqlTypeName = LONGSTR_NULLABLE._sqlTypeName;
...
export function createEnum(enumName: string, values, nullable = false) {
  ...
  const res = {
    ...
    _validateType: function(value, valType, futureFeed?) {
      if (!IDSTR._validateType(value, valType)) {
        return false;
      }
